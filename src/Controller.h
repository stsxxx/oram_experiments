...
public:
    ...
    rs* myrs = nullptr;
    rs* myrs_pos1 = nullptr;
    rs* myrs_pos2 = nullptr;
        ...
        if (pending.size()) {
            ...
            if (req.depart <= clk) {
                ...
                if(req.name == "data"){
                    ...
                    if(myrs_pos1 != NULL && myrs_pos1->type != "pathoram" && req.metadata == false){
                        myrs_pos1->mshr_remove((long long) req.addr);
                    }      
                }
                else if(req.name == "pos2"){
                    if(myrs_pos2 != NULL && myrs_pos2->type != "pathoram" && req.metadata == false){
                        myrs_pos2->mshr_remove((long long) req.addr);
                    }      
                }
